...
interface IGetDbRecordByID {
  ...
  <T, P>(recordString: string, id: string, normaliser: (data: T) => P): Promise<P>;
}

export const getDbRecordById: IGetDbRecordByID = <T, P = undefined>(recordString: string, id: string, normaliser?: (data: T) => P) => {
  return new Promise(async (resolve, reject) => {
    try {
      const ref = await fireDb.ref(`${recordString}/${id}`).once('value');

      resolve(typeof normaliser === 'function' ? normaliser(ref.val()) : ref.val());
    } catch (error) {
      reject(error);
    }
  });
...
export const getDbRecords: IGetDbRecords = <T, P = undefined>(recordString: string, normaliser?: (data: T) => P) => {
  return new Promise(async (resolve, reject) => {
    try {
      const ref = await fireDb.ref(recordString).once('value');
      const data = typeof normaliser === 'function' ? normaliser(ref.val()) : normaliseObjectKeysToArray(ref.val());
...
export const updateRecord = <T>(recordString: string, id: string, data: T): Promise<T> => {
  return new Promise(async (resolve, reject) => {
    try {
...
export const addRecord = <T>(recordString: string, data: T, includeId: boolean): Promise<T> => {
  return new Promise(async (resolve, reject) => {
    try {
...
export const removeRecord = (recordString: string, id: string): Promise<void> => {
  return new Promise(async (resolve, reject) => {
    try {
